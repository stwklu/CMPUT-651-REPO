...
class LogisticRegression:
	...
	def __init__(self, X_train, X_val, X_test, y_train, y_val, y_test):
		...
		self.lamb = 0.001
		# parameters
		self.W = np.random.uniform(-0.5, 0.5, (self.n_features, 1))
		self.W_opt = np.zeros((self.n_features, 1))
		self.best_acc = -1
		# Training log
	...
	def train(self):
		...
		for e in range(self.n_epochs):
			...
			for b in range(x_batches.shape[0]):
				grad = self.gradient(x_batches[b], y_batches[b], self.regularization)
	...
	def loss(self, x, y, optimal=False):
		...
		m = x.shape[0]
		if optimal:
			y_pred = self.sigmoid(np.dot(x, self.W_opt))
		else:
			y_pred = self.sigmoid(np.dot(x, self.W))
		return -(1/m) * np.sum(y * np.log(y_pred) + (1-y) * np.log(1-y_pred))

	def gradient(self, x, y, regularization=None):
		'''
		Gradient for give batch x and label y
		...
		if regularization == "l1":
			l = l + self.lamb * np.sign(self.W)
		if regularization == "l2":
			l = l + 2 * self.lamb * self.W
		else:
			pass
		return l
	...
	def get_accuracy(self, x, y, optimal=False):
		...
		'''
		if optimal:
			prob = self.sigmoid(np.dot(x, self.W_opt))
		else:
			prob = self.sigmoid(np.dot(x, self.W))
		prob = prob >= 0.5
		acc = np.sum(prob==y) / x.shape[0]
		return acc
...
def main():
	...
	model = LogisticRegression(train_features, val_features, test_features, y_train, y_val, y_test)
